document.addEventListener('DOMContentLoaded', () => {
  ...
  const pendingAppointmentsTableBody = document.getElementById('pendingAppointmentsTableBody');
  ...
  function exportFromAppointment(appointmentId) {
    ...
    if (appointment) {
      ...
      document.getElementById('exportDoctorId').value = appointment.doctor_id;
  ...
  async function loadPendingAppointments() {
    try {
      const response = await window.meducoAPI.getAppointments({ status: 'pending' });
      if (response.success) {
        const pendingAppointments = response.data.appointments;
        populatePendingAppointmentsTable(pendingAppointments);
      } else {
        console.error('Failed to load pending appointments:', response.message);
      }
    } catch (error) {
  ...
  function populatePendingAppointmentsTable(appointments) {
    if (!pendingAppointmentsTableBody) return;
    pendingAppointmentsTableBody.innerHTML = '';
    if (!appointments || appointments.length === 0) {
      pendingAppointmentsTableBody.innerHTML = '<tr><td colspan="7" style="text-align: center;">No pending appointments</td></tr>';
      return;
    }
    appointments.forEach(appointment => {
      const row = document.createElement('tr');
      row.innerHTML = `
        ...
        <td>
          <button class="btn btn-primary approve-btn" data-id="${appointment.id}">Approve</button>
          <button class="btn btn-success done-btn" data-id="${appointment.id}">Mark Done</button>
        </td>
      `;
      pendingAppointmentsTableBody.appendChild(row);
    });

    // Bind approve and done buttons
